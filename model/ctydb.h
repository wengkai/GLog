...
class CtyDB : public QObject {
  ...
  public:
    static constexpr const char *DEFAULT_ONLINE_DATA =
        "https://www.country-files.com/bigcty/cty.dat";
    std::pair<bool, QString /*error msg*/>
    loadLocalDB(const QString &filename = QStringLiteral(":/assets/cty.dat"));
    std::pair<std::shared_ptr<CtyEntry>, QString /*hint*/>
    lookUpCallSign(const QStringView &call) const;
    std::pair<std::shared_ptr<CtyEntry>, QString /*hint*/>
    lookUpCallSign(const QString &call) const;
    inline std::pair<std::shared_ptr<CtyEntry>, QString /*hint*/>
    lookUpCallSign(const std::string &call) const {
