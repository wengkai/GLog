...
GLogApplication::GLogApplication(QWidget *parent)
    ...
    connect(tableview, &DropAbleTableView::newViewWithRowsSignal, model,
            &AdifModel::newViewWithRows);
    ...
    connect(ui->actionMapCallSignInView, &QAction::triggered, [=]() {
        if (!CtyDB::instance()->ready()) {
            QMessageBox::warning(this, tr("Warning"), tr("The mapping database has not ready."),
                                 QMessageBox::StandardButton::Ok);
            return;
        }
        auto button1 = QMessageBox::question(this, tr("Warning"),
        ...
        auto button2 = QMessageBox::question(
            this, tr("Warning"),
...
void GLogApplication::extractZip(const QString &zipPath, const QString &extractDir,
    ...
    if (!proc.waitForFinished() || proc.exitCode() != 0) {
        ...
        QString _7zipbin =
            QStandardPaths::writableLocation(QStandardPaths::AppDataLocation) + "/7zip";
        ...
        };
        {
            ...
            if (!_7za_file.exists()) {
                ...
                _7zr_proc.start(_7zrExePath,
                                {"x", QDir::toNativeSeparators(_7za7zPath),
                                 "-o" + QDir::toNativeSeparators(_7zipbin), "7za.exe", "-y"});
                if (!_7zr_proc.waitForFinished() || _7zr_proc.exitCode() != 0) {
                    throw std::runtime_error(tr("Extraction failed").toStdString());
                }
            }
            QProcess _7za_proc;
            _7za_proc.start(_7zaExePath, {"x", QDir::toNativeSeparators(zipPath),
                                          "-o" + QDir::toNativeSeparators(extractDir), "-y"});
...
void GLogApplication::saveAsAction() {
    ...
    saveAsFile(QFileDialog::getSaveFileName(this, tr("Save As"), "",
                                            tr("Adif Files (*.adi *.adif);;Csv Files (*.csv)")));
}
...
void GLogApplication::updateFccDatabase() {
    ...
    auto button1 = QMessageBox::question(
        this, tr("FCC Database"),
        tr("This action will download complete FCC database (l_amat.zip) and make a simple one for "
    ...
    QString oldDBPath =
        dbPath + "." + QDateTime::currentDateTime().toString("yyyyMMddHHmmss") + ".bak";
    if (existsDB) {
        auto oldDB = QFile(dbPath);
        if (!oldDB.rename(oldDBPath)) {
            emit warning(tr("FCC Database"), tr("Cannot remove old database file"),
                         QMessageBox::StandardButton::Ok);
            return;
        }
    }
    ...
    if (zipInfo.exists()) {
        auto button2 = QMessageBox::question(
            this, tr("FCC Database"), tr("Found %1.\nSkip download?").arg(zipPath),
            QMessageBox::StandardButton::Yes | QMessageBox::StandardButton::No);
    ...
    GLogConcurrent::makeFuture([=](QPromise<void> &promise) {
        ...
        QString connectionName = QString("fcc_build_%1").arg(quintptr(QThread::currentThreadId()));
        {
            ...
            if (!db.open()) {
                throw std::runtime_error(tr("Database open failed").toStdString());
            }
            ...
            if (enFile.open(QIODevice::ReadOnly | QIODevice::Text)) {
                while (!enFile.atEnd()) {
                    ...
                    if (enCount % 10000 == 0) {
                        emit showMessage(tr("Processing EN.dat: %1").arg(enCount));
                    }
                    ...
                    if (stateCandidate.length() == 2 &&
                        stateCandidate == stateCandidate.toUpper()) {
                        if (usi > 0) {
            ...
            if (hdFile.open(QIODevice::ReadOnly | QIODevice::Text)) {
                ...
                while (!hdFile.atEnd()) {
                    ...
                    if (hdCount % 10000 == 0) {
                        emit showMessage(tr("Processing HD.dat: %1").arg(hdCount));
                    }
